...
def LSTM_model(path):
    ...
    #BIRNN 两层双向RNN模型
    # 前向网络
    hidden_units = config.FLAGS.lstm_hidden_units
    single_cell_fw = [tf.nn.rnn_cell.LSTMCell(size) for size in [hidden_units*2, hidden_units]]
    cell_fw = tf.nn.rnn_cell.MultiRNNCell(single_cell_fw)

    # 反向网络
    single_cell_bw = [tf.nn.rnn_cell.LSTMCell(size) for size in [hidden_units*2, hidden_units]]
        ...
        for i in range(config.FLAGS.epoch):
            ...
            for j in range(num_batchs):
                ...
                if j % config.FLAGS.disp_freq == 0:
                    ...
                    total_loss = 0
            dh.batch_sum = 0
            saver.save(sess, 'ckpt/Multi_BIRNN.ckpt', global_step=i + 1)
            print("epoch %d;Time taken:%s" % (i, datetime.now() - epochTime))
